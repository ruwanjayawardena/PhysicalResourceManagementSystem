...
<html lang="en">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12 col-sm-12">
                        <h4 class="text-uppercase admin-page-heading"><i class="fas fa-file"></i> GRN Info - Institute &nbsp;&nbsp;<button class="btn btn-light d-print-none" onclick="goDashboard();"><i class="fas fa-chevron-circle-left"></i>&nbsp;Back</button></h4>                       
                    </div>
                </div>
                            ...
                            <table id="tblGrn" class="table table-bordered table-hover" style="width:100%">
                                <thead class="thead-dark">
                                    <tr>                                                       
                                        ...
                                        <th>Receiving Created Date/Time</th>                                        
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
    ...
    <script>
		function tblGrn(callback) {
			var tbldata = "";
			$.post('controllers/grnController.php', {action: 'getAllGrnInstitute'}, function (e) {
				if (e === undefined || e.length === 0 || e === null) {
					tbldata += '<tr>';
					tbldata += '<td colspan="8" class="bg-danger text-center"> -- Institute GRN not available -- </td>';
					tbldata += '</tr>';
					...
					$.each(e, function (index, qdt) {
						...
						if (parseInt(qdt.grn_status) == 0) {
							...
							tbldata += '<button class="btn btn-warning btn_prepare" value="' + qdt.grn_id + '-' + qdt.grn_issue_id + '"><i class="fas fa-edit"></i> Prepare </button>';
//							tbldata += '<button class="btn btn-danger btn_delete" value="' + qdt.grn_id + '"><i class="fas fa-trash-alt"></i></button>';
							tbldata += '</div>';
						} else {
							tbldata += '<button class="btn btn-sm btn-primary btn_viewReceipt" value="' + qdt.grn_id + '-' + qdt.grn_issue_id + '"><i class="fas fa-file"></i> View Receipt</button>';
						}
						tbldata += '</td>';
						tbldata += '<td></td>';
				...
				$('.btn_prepare').click(function () {
					...
					var grn_id = ar[0];
					var issu_id = ar[1];
					window.location.href = 'grnInstituteItem.php?grn_id=' + grn_id + '&issu_id=' + issu_id;
				});
				...
				$('.btn_viewReceipt').click(function () {
					...
					var grn_id = ar[0];
					var issu_id = ar[1];
					window.location.href = 'viewGrnInstituteReceipt.php?grn_id=' + grn_id + '&issu_id=' + issu_id;
				});
